...
class CorporationSpeculationOrder(AbstractSpeculation):
	...
	def resolve(self):
		...
		if ladder.index(self.corporation) + 1 == self.rank:
			...
			self.player.game.add_event(event_type=Game.SPECULATION_WIN, data='', players=[self.player])
		else:
			# Failure
			...
			self.player.game.add_event(event_type=Game.SPECULATION_LOST, data='', players=[self.player])
