...
class ModelsTest(EngineTestCase):
	...
	def test_game_add_newsfeed_from_template(self):
		...
		"""
		self.g.add_newsfeed_from_template(category=Newsfeed.MATRIX_BUZZ, path='datasteal/c3/success')

		newsfeed = self.g.newsfeed_set.get()

		self.assertEqual(newsfeed.content, read_file_from_path('%s/newsfeeds/4-matrix-buzz/datasteal/c3/success/1.md' % settings.CITY_BASE_DIR))
	...
	def test_game_add_newsfeed_from_template_read_directory(self):
		...
		for i in range(1, 10):
			...
			try:
				expected_content = read_file_from_path('%s/newsfeeds/4-matrix-buzz/datasteal/c3/success/%s.md' % (settings.CITY_BASE_DIR, i))
				self.assertEqual(newsfeed.content, expected_content)
			except IOError:
				expected_content = read_file_from_path('%s/newsfeeds/4-matrix-buzz/datasteal/c3/success/_.md' % settings.CITY_BASE_DIR)
				self.assertEqual(newsfeed.content, expected_content)
				break
		else:
