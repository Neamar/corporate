...
class ModelTest(EngineTestCase):
	...
	def test_game_build_resolution_message_Jules(self):
		...
		n2.save()
		n2.recipient_set.add(p2)
		self.g.build_resolution_message()
		#p2_receive_only_one_message
		self.assertEqual(Message.objects.filter(recipient_set=p2).exclude(flag=Message.GLOBAL_NOTE).count(),1)
		#p2_reveive_message_T1_and_T2
		self.assertEqual(Message.objects.filter(recipient_set=p2).exclude(flag=Message.GLOBAL_NOTE)[0].content,u"### Résolution du tour 1 ###\n\n\n## T1\n* C1\n\n## T2\n* C2\n")
